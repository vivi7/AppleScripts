...
def verifyCommands(com):
	...
	strError = "Per eseguire questo script sono necessari i seguenti comandi:"
	...
	for c in commands:
		...
		if prog[0] not in table:
			...
			strError=strError+"\n    * " + prog[0]
			if len(prog)>1:
				strError=strError+" (del pacchetto \""+ prog[1]+"\")"
...
def ProcessFile(f):
	...
	if todo==True:
		...
		os.spawnvp(os.P_WAIT, program, (program,) +  tuple(newArguments))
